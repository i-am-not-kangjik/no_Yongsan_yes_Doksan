...
def remove_irrelevant_products(df):
    ...
    df['crawling'] = df['crawling'].apply(lambda x: 0 if x == '당근마켓' else (1 if x == '중고나라' else (2 if x == '번개장터' else x)))
...
def etl():
    ...
    source_key = f'daangn/당근마켓_{current_time}.csv'
    target_bucket = 'doksan-data'
    target_key = f'Preprocessing/당근마켓_{current_time}.csv'
    ...
    for index, row in df.iterrows():
        ...
        '''
        params = (row['product_grade_id'], row['phone_cost_id'], row['price'], row['favoriteCount'], row['viewCount'], row['hours'], row['crawling'])
