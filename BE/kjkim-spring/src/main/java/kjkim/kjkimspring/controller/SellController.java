...
public class SellController {
    ...
    @GetMapping("/sell")
    public Page<Sell> sell(@RequestParam(value = "page", defaultValue = "0") int page) {
        return this.sellService.getList(page);
    }

    @Transactional
    @GetMapping(value = "/sell/{id}")
    public Sell detail(@PathVariable("id") Integer id, CommentForm commentForm) {
        Sell sell = this.sellService.getSell(id);
        sell.increaseViewCount(); // Increase view count
        this.sellService.saveSell(sell); // Save the updated Sell object
    ...
    // The rest of your controller methods go here, each one returning data directly
    // instead of view names.

    // Note that methods for creating, modifying, and deleting a sell will need to
    // take JSON data as input. This is typically done with a @RequestBody annotation.
    // Also, instead of redirecting, they should return the created/updated/deleted sell.
}
